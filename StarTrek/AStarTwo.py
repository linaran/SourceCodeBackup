...
def nextNodeSpecial(jeanLuc, incZero, incOne):
    ...
    if universe[nextY][nextX][0] == "T":
        return(
            ...
            HeuristicsGetter.getHeuristic(universe, (nextY, nextX), goal),
            jeanLuc
        )
...
for i in range(0, MAXHeight):
    for j in range(0, MAXWidth):
        ...
        if universe[i][j] == "C":
            goal = (i, j, 0, 0, None)
jeanLuc = (jeanLuc[0], jeanLuc[1], 0, HeuristicsGetter.getHeuristic(universe, jeanLuc, goal), None)
